...
## Optionality: three flags, four signals, and one source that cannot merge

A translated attribute carries the tfplugin trio: Required, Optional,
Computed. The interesting one is the pair, Optional AND Computed, which
means "you may set this, and if you do not, the provider will". It is the
honest description of most cloud resource fields, and it is the one
combination a single schema cannot express.

Translation sees one schema at a time, and one schema carries only three
signals: whether a field is required, whether it is read-only, whether it
is write-only. Those map cleanly onto the trio, and none of them says
"defaulted". The combination comes from a MERGE instead: a create-request
schema says a field is optional, a read-response schema says it comes
back, and putting the two together is what proves the provider supplies
it. Three of the four sources describe request and response separately,
so all three merge.

CloudFormation does not. Its registry publishes one schema per resource,
so there is no second schema, so the merge never ran, so the combination
was structurally unreachable. Every optional AWS property was declared as
though nothing filled it in.

That is not a cosmetic inaccuracy. A consumer that decides "an omitted
attribute was deliberately removed" from the absence of Computed will
strip settings a cloud provider put there. That is not hypothetical: a
modify that never mentioned an SQS queue's visibility timeout planned to
remove it.

The registry does not carry the missing information anywhere. Under 2% of
optional properties declare a JSON Schema `default`, another 3% hint at
one in prose, and the resource that motivated the investigation has
neither for any of its properties. So it is asserted rather than derived:
for the CloudFormation source, an optional property is declared
server-defaultable. HashiCorp's own generated AWS Cloud Control provider,
working from the identical registry, does the same thing.

Asserting it is safe in one direction and not the other, which is the
whole argument. Claiming a value may be supplied when nothing supplies it
is conservative: a consumer keeps something it could have discarded.
Claiming nothing supplies it when something does destroys real
configuration.

Read-only properties keep the narrower declaration, Computed and NOT
Optional, and that distinction is load-bearing downstream rather than
pedantic. "The provider owns this outright" and "the provider may supply
this" answer different questions: a value appearing where none was
recorded is expected for the first and is real drift for the second.
Consumers that had collapsed those into one question had to separate them
BEFORE this change shipped, or ~17,600 attributes would have quietly
stopped reporting drift the day the schema got more accurate.
...
## The binary that serves a snapshot is a floor, not a pin

A snapshot records which build of the provider binary cut it. That sounds
like provenance trivia and turned out to be the difference between a
bugfix reaching users and not.

The obvious reading is that the recorded version is the version to run. It
is not what the field means: it records a build known to work, not the
oldest that would, and it was renamed once already because the earlier
name promised a floor it never computed. Running it exactly reads the
field as something it says it is not.

The cost only becomes visible when a bug is found in the binary rather
than in a schema. A serialization bug made a whole class of attribute
unencodable, surfacing as a resource that was really created in the cloud
and then failed on the way back. It was fixed and released within the
hour, and reached nobody: every published snapshot, across all eight
providers, recorded a build that predated it, and the bug lived in code
every source shares.

Under the exact reading, shipping that one-line fix means regenerating and
re-releasing eight repositories, each of which then owes a schema version
bump for a change that touched no schema. The fix is done and the delivery
is a migration.

So the recorded version is treated as a floor, and the newest build
sharing its major serves the snapshot. The major is where it stops
guessing, because the snapshot format version is the real compatibility
contract and all eight published snapshots share one. A tighter bound was
considered and rejected for being nearly useless: it would have helped
only whichever provider happened to sit at the newest minor.

This is deliberately NOT the same as resolving "latest" for a pinned
input, a thing this system refuses to do elsewhere and still refuses. The
pinned, reviewed, hashed input is the schema snapshot version, and it
still resolves exactly. What moves is which build of the server process
reads it, which was never something anyone reviewed.

It does cost real reproducibility, and the honest version is worth stating
rather than arguing away: two runs against the same pinned snapshot can
use different binaries, and a different binary can in principle read
differently. Three things bound it. The binary's own checksum is recorded
either way, so what actually ran is recoverable rather than inferred. The
bound never crosses a major. And an escape hatch restores exact
acquisition for anyone reconstructing an old run precisely.

The trade was made knowingly. A fix that cannot reach any user is a worse
property than a rebuild that needs a flag to be exact.
