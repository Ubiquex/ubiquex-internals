...
`scripts/resource-reference-gen/provenance_check.py` and
`coverage_check.py` for the consuming side (a private repo — not tracked
by this site's own drift check, which needs public, unauthenticated
clone access to whatever it mirrors; read directly on GitHub for the
current version).
